...
const WithdrawalGoalButton = ({ refresh }) => {
    const GOAL_API_WITHDRAWAL_URL = API_URLS_CONFIG.BASE_URL + "/api/goal/withdrawal";
    ...
    const addGoal = async () => {
        try {
            const response = await axios({method:'post', url: GOAL_API_WITHDRAWAL_URL, data: {name: nameOfGoal, withdrawal: withdrawal},
                headers: {
            ...
            if (response.status === 200) {
                setSuccessful(true)
                setMessage("Money was successfully withdraw from your Financial Goal!")
                refresh();
                setUnsuccessful(false)
            }
            ...
            if (error.response) {
                ...
                if (status === 404) {
                    ...
                    setErrorMessage("Your current balance of this goal is to low!")
                }
            }
        }
    };
                    ...
                    <Transition.Child as={Fragment}
                        ...
                        <div
                            ...
                            <Dialog.Title
                                ...
                                Withdrawal money from your Financial Goal!
                            </Dialog.Title>
...
export default WithdrawalGoalButton;